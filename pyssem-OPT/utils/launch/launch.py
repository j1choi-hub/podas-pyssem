...
def launch_func_lambda_fun(t, h, species_properties, scen_properties):
    ...
    if dambrosio == True:
        ...
        # Find the index of the shell closest to the specified altitude
        for alt in scen_properties.R0_km[scen_properties.R0_km>=800]:
            ...
            # Fill the row corresponding to the nearest altitude with the launch rate per time step
            launch_rate_matrix[h_inds, :] += launch_rate_per_year

    opt_launch = False
    if opt_launch == True:
        launch_file_path = os.path.join('utils', 'launch', 'data', 'launch_36.csv')
        launch_rate_per_year = pd.read_csv(launch_file_path)
        launch_rate_per_year = launch_rate_per_year["Launches"].tolist()
        for idx in range(0, len(launch_rate_matrix)):
            launch_rate_matrix[idx, :] = launch_rate_per_year[idx]

    print(launch_rate_matrix[:, 99])
    return launch_rate_matrix
...
def BASE_init_model(scen_properties, file_path=None):
    ...
    if file_path == None:
        ...
        x0_params = [[650, 300 ** 2, 850], [300, 150 ** 2, 400], [450, 300 ** 2, 125]]
...
def ADEPT_traffic_model(scen_properties, file_path):
    ...
    #T = T[((T.species_class == "S") & (T.mass <= 600)) | ((T.species_class == "N") & (T.mass <= 1))]
