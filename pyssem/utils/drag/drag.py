...
def densityexp(h):
    ...
    for h0, p0, H in layers:
        ...
        if h0 >= 500:
            incr = 100
        mask = (h >= h0) & (h < h0 + incr)
        p[mask] = p0 * np.exp((h0 - h[mask]) / H) #* 1e9
...
def drag_func_exp(t, h, species, scen_properties):
    ...
    print("PER YEAR", seconds_per_year, time_steps_per_year)
    ...
    if species.drag_effected:
        ...
        for k in range(scen_properties.n_shells):
            ...
            if k < scen_properties.n_shells - 1:
                ...
                rvel_upper[k] = species.beta * sqrt(scen_properties.mu * scen_properties.R0[k+1]) * seconds_per_year #/ time_steps_per_year 
            ...
            rvel_current[k] = -species.beta * np.sqrt(scen_properties.mu * scen_properties.R0[k]) * seconds_per_year #/ time_steps_per_year 
            
            upper_term[k] = n0 * rvel_upper[k] / scen_properties.deltaH[min(k+1, scen_properties.n_shells-1)]
            current_term[k] = rvel_current[k] / scen_properties.deltaH[k] * species.sym[k]
