...
def evolve_bins(m1, m2, r1, r2, dv, binC, binE, binW, LBdiam, RBflag = 0, fragment_spreading=False, n_shells=0, R02 = None): # eventually add stochastic ability
    ...
    :param r1: Radius of Object 1 [km]
    :type r1: int or float
    :param r2: Radius of Object 2 [km]
    ...
    r1 = r1*1000 # convert r1 unit to meters
    r2 = r2*1000 # convert r2 unit to meters
...
def process_species_pair(args):
    ...
    gammas = Matrix(scen_properties.n_shells, 2, lambda i, j: 1)
    gammas_sqrt = Matrix(scen_properties.n_shells, 2, lambda i, j: 1) #for E-M simulations

    # Calculate the number of fragments made for each debris species
    frags_made = np.ones((len(scen_properties.v_imp2), len(debris_species)))
    ...
    #print("---------------------- source:", s1.sym_name, "sink:", s2.sym_name," -------------------------------------------")
    source_sinks = [s1, s2]
    ...
    # Create symbols for debris-generation
    n_f = Matrix(scen_properties.n_shells, len(debris_species), lambda i, j: 1)
    n_f[:, 0] = symbols(f'n_f:{scen_properties.n_shells}')

    model_type = "MOCAT-3" #"MOCAT-3"  # "JASON", "PODAS"

    if model_type == "MOCAT-3":
        if s1.active and s2.active:
            jump_size = 1
            gammas[:, 0] = gammas[:, 0] * (-jump_size*s1.alpha_active)
            gammas[:, 1] = gammas[:, 1] * 0
            gammas_sqrt[:, 0] = gammas_sqrt[:, 0] * (-jump_size * np.sqrt(s1.alpha_active))
            gammas_sqrt[:, 1] = gammas_sqrt[:, 1] * 0
            frags_made[:, 0] = frags_made[:, 0] * s1.alpha_active
            frags_made[:, 1] = frags_made[:, 1] * 0
        elif s1.active and not s2.active:
            jump_size = 1
            gammas[:, 0] = gammas[:, 0] * (-jump_size*s1.alpha -jump_size*s2.delta)
            gammas[:, 1] = gammas[:, 1] * 0
            gammas_sqrt[:, 0] = gammas_sqrt[:, 0] * (-jump_size * np.sqrt(s1.alpha) - jump_size * np.sqrt(s2.delta))
            gammas_sqrt[:, 1] = gammas_sqrt[:, 1] * 0
            frags_made[:, 0] = frags_made[:, 0] * s1.alpha
            frags_made[:, 1] = frags_made[:, 1] * s2.delta
        elif s2.active and not s1.active:
            jump_size = 1
            gammas[:, 1] = gammas[:, 1] * (-jump_size*s1.alpha - jump_size* s2.delta)
            gammas[:, 0] = gammas[:, 0] * 0
            gammas_sqrt[:, 1] = gammas_sqrt[:, 1] * (-jump_size * np.sqrt(s1.alpha) - jump_size * np.sqrt(s2.delta))
            gammas_sqrt[:, 0] = gammas_sqrt[:, 0] * 0
            frags_made[:, 0] = frags_made[:, 0] * s1.alpha
            frags_made[:, 1] = frags_made[:, 1] * s2.delta
        elif not s1.active and not s2.active:
            if (len(s1.pmd_linked_species) == 0) and (len(s2.pmd_linked_species) == 0):
                gammas[:, 0] = gammas[:, 0] * 0
                gammas[:, 1] = gammas[:, 1] * 0
                gammas_sqrt[:, 0] = gammas_sqrt[:, 0] * 0
                gammas_sqrt[:, 1] = gammas_sqrt[:, 1] * 0
                frags_made[:, 0] = frags_made[:, 0] * 1
                frags_made[:, 1] = frags_made[:, 1] * 0
            elif (len(s1.pmd_linked_species) > 0) and (len(s2.pmd_linked_species) == 0):
                gammas[:, 0] = gammas[:, 0] * (-1)
                gammas[:, 1] = gammas[:, 1] * 0
                gammas_sqrt[:, 0] = gammas_sqrt[:, 0] * (-1)
                gammas_sqrt[:, 1] = gammas_sqrt[:, 1] * 0
                frags_made[:, 0] = frags_made[:, 0] * 1
                frags_made[:, 1] = frags_made[:, 1] * 0
            elif (len(s1.pmd_linked_species) == 0) and (len(s2.pmd_linked_species) > 0):
    ...
    if model_type == "PODAS":
        if s1.active and s2.active:
            gammas[:, 0] = gammas[:, 0] * (-1*s1.alpha_active)
            gammas[:, 1] = gammas[:, 1] * (-1*s1.alpha_active)
            gammas_sqrt[:, 0] = gammas_sqrt[:, 0] * (-1 * np.sqrt(s1.alpha_active))
            gammas_sqrt[:, 1] = gammas_sqrt[:, 1] * (-1 * np.sqrt(s1.alpha_active))
            frags_made[:, 0] = frags_made[:, 0] * s1.alpha_active
            frags_made[:, 1] = frags_made[:, 1] * 0
        elif s1.active and not s2.active:
            gammas[:, 0] = gammas[:, 0] * (-1*(s1.alpha + s2.delta))
            gammas_sqrt[:, 0] = gammas_sqrt[:, 0] * (-1 * (np.sqrt(s1.alpha) + np.sqrt(s2.delta)))
            if (len(s2.pmd_linked_species) == 0):
                gammas[:, 1] = gammas[:, 1] * 0
                gammas_sqrt[:, 1] = gammas_sqrt[:, 1] * 0
            else:
                gammas[:, 1] = gammas[:, 1] * (-1*(s1.alpha))
                gammas_sqrt[:, 1] = gammas_sqrt[:, 1] * (-1 * np.sqrt(s1.alpha))
            frags_made[:, 0] = frags_made[:, 0] * s1.alpha
            frags_made[:, 1] = frags_made[:, 1] * s2.delta
        elif s2.active and not s1.active:
            gammas[:, 1] = gammas[:, 1] * (-1*(s2.alpha + s1.delta))
            gammas_sqrt[:, 1] = gammas_sqrt[:, 1] * (-1 * (np.sqrt(s2.alpha) + np.sqrt(s1.delta)))
            if (len(s1.pmd_linked_species) == 0):
                gammas[:, 0] = gammas[:, 0] * 0
                gammas_sqrt[:, 0] = gammas_sqrt[:, 0] * 0
            else:
                gammas[:, 0] = gammas[:, 0] * (-1*(s2.alpha))
                gammas_sqrt[:, 0] = gammas_sqrt[:, 0] * (-1 * np.sqrt(s2.alpha))
            frags_made[:, 0] = frags_made[:, 0] * s1.alpha
            frags_made[:, 1] = frags_made[:, 1] * s2.delta
        elif not s1.active and not s2.active:
            if (len(s1.pmd_linked_species) == 0) and (len(s2.pmd_linked_species) == 0):
    ...
    if model_type == "JASON":
        if s1.active and s2.active:
            ...
            gammas[:, 1] = gammas[:, 1] * (-1*(s1.alpha + s2.delta))
            gammas[:, 0] = gammas[:, 0] * 0
            frags_made[:, 0] = frags_made[:, 0] * s1.alpha
            frags_made[:, 1] = frags_made[:, 1] * s2.delta
        elif not s1.active and not s2.active:
    ...
    for i, species in enumerate(debris_species):
        #print("i, species", i, species.sym_name, Matrix(frags_made[:, i]) )
        ...
        new_column = n_f[:, i].multiply_elementwise(frags_made_sym)
        ...
        gammas_sqrt = gammas_sqrt.col_insert(2 + i, new_column.applyfunc(sqrt))
    ...
    print("Species pairing for ", s1.sym_name, s2.sym_name, "and sk ", [sk.sym_name for sk in source_sinks])
    #print("Gamma: ",gammas)
    if scen_properties.fragment_spreading:
        return SpeciesPairClass(s1, s2, gammas, gammas_sqrt, source_sinks, scen_properties, model_type, alt_nums)
    else:
        return SpeciesPairClass(s1, s2, gammas, gammas_sqrt, source_sinks, scen_properties, model_type)
