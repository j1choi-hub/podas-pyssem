...
class SpeciesProperties:
    def __init__(self, properties_json=None):
        ...
        self.mass_ub = 1000  # Upper bound of mass class for object binning (exclusive, except for top mass bin, which is inclusive)
        self.radius = None 
        self.A = None  # km^2
        self.amr = None  # km^2/kg
        self.beta = None  # km^2/kg, ballistic coefficient (Cd * A / mass)
        self.B = None # 
        ...
        self.delta = None # ratio of the density of disabling to lethal debris
...
class Species:
    ...
    def add_multi_property_species(self, species_properties):
        ...
        for i in range(len(multi_species_list)):
            if multi_species_list[i].mass_lb == None:
    ...
    def create_symbolic_variables(self, n_shells: int):
        ...
        for species_group in self.species.values():
            for species in species_group:
                ...
                sym_name_r = species.sym_name.replace('.', 'p') # P means decimal point
                species.sym = Matrix(symbols([f'{sym_name_r}_{i+1}' for i in range(n_shells)]))
