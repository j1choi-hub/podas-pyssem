...
class ScenarioProperties:
    def __init__(self, start_date: datetime, simulation_duration: int, steps: int, min_altitude: float,
                 max_altitude: float, v_shells: float, launch_function: str,
        ...
        self.mu = 398601  # earth's gravitational constant km^3/s^2
        self.re = 6378.1366  # radius of the earth [km]

        # Shell Count
        R0 = [self.min_altitude]
        # while R0[-1] < self.max_altitude:
        while len(R0) < 11:
            R0.append((3 / 4 * (1 / pi) * v_shells + R0[-1] ** 3) ** (1 / 3))
        R0 = np.array(R0)
        self.n_shells = len(R0) - 1
        self.V = [v_shells] * self.n_shells
        print("SHELL COUNT: ", self.n_shells)
        ...
        self.deltaH = np.diff(self.R0_km)  # [0]  # thickness of the shell [km]
        R0 = (self.re + R0)  # Convert to meters and the radius of the earth
        ...
        self.Dhl = self.deltaH  # thickness of the shell [m]
        self.Dhu = -self.deltaH  # thickness of the shell [m]
        ...
        self._numeric_funcs_cached = None
    ...
    def future_launch_model(self, FLM_steps):
        ...
        for species_group in self.species.values():
            for species in species_group:
                ...
                # print(species.sym_name, "species_FLM ", species_FLM)

                # Convert spec_FLM to interpolating functions (lambdadot) for each shell
                # Remember indexing starts at 0 (40th shell is index 39)
                species.lambda_funs = []
                launch_rate_matrix = np.zeros((self.n_shells, len(scen_times)))
                if species.active == True:
                    for shell in range(self.n_shells):
                        ...
                        if np.all(y == 0):
                            ...
                            launch_rate_matrix[shell, :] = y
                            # lambda_funs로 간 다음에 interpolation 하는 것 같음
                            # species.lambda_funs.append(np.array(y))
                # print(species.sym_name, "Species.lambdafuns: ", launch_rate_matrix, )
                # print([sum(x[i] for x in launch_rate_matrix) for i in range(len(scen_times))])
                species.lambda_funs = launch_rate_matrix.copy()
    ...
    def initial_pop_and_launch(self, baseline=False):
        ...
        if baseline:
            print("baseline")
            init_file_path = os.path.join('utils', 'launch', 'data', 'x0_20.csv')
            self.x0 = BASE_init_model(self)

        else:
            launch_file_path = os.path.join('utils', 'launch', 'data',
                                            'x0_launch_repeatlaunch_2018to2022_megaconstellationLaunches_Constellations.csv')

            # Check to see if the data folder exists, if not, create it
            if not os.path.exists(os.path.join('utils', 'launch', 'data')):
                os.makedirs(os.path.join('utils', 'launch', 'data'))
    ...
    def build_model(self):
        ...
        time_steps_per_year = self.steps / self.simulation_duration
        ...
        self.full_lambda = []
        self.lam = sp.zeros(self.n_shells, self.species_length)
        lam_launch = symbols(f'lam_:{self.n_shells + 1}')
        ...
        for k in range(self.n_shells):
            self.all_symbolic_vars.append(lam_launch[k])
        ...
        for i, species in enumerate(species_list):
            ...
            #     lambda_expr = species.lambda_funs #species.launch_func(self.scen_times, self.HMid, species, self)
            # else:
            ...
            for k in range(self.n_shells):
                # self.all_symbolic_vars.append(lam_launch[k])
                if species.active == True:
                    self.lam[k, i] = lam_launch[k]
                if species.active == False:
                    self.lam[k, i] = 0
        ...
        self.equations = self.lam + self.full_Cdot_PMD + self.full_coll
        ...
        if not self.time_dep_density:  # static density
            ...
            rho = self.density_model(t, self.R0_km, self.species, self)
            ...
            self.drag_upper_with_density = self.drag_term_upper.multiply_elementwise(upper_rho)
            self.drag_cur_with_density = self.drag_term_cur.multiply_elementwise(current_rho)
            self.full_drag = self.drag_upper_with_density + self.drag_cur_with_density
    ...
    def get_eqn(self):
        equations_flattened = [self.equations[i, j] for j in range(self.equations.cols) for i in
                               range(self.equations.rows)]
        print("equation flattened ", equations_flattened)
        equations = [sp.lambdify(self.all_symbolic_vars, eq, 'numpy') for eq in equations_flattened]
        return equations

    # def generate_symbolic_equations(self, t):
    #     equations = sp.zeros(self.n_shells, self.species_length)
    #     equations += self.lam
    #     equations += self.full_Cdot_PMD
    #     equations += self.full_coll
    ...
    def run_model(self):
        ...
        if self.time_dep_density:
            ...
            drag_upper_flattened = [self.drag_term_upper[i, j] for j in range(self.drag_term_upper.cols) for i in
                                    range(self.drag_term_upper.rows)]
            drag_current_flattened = [self.drag_term_cur[i, j] for j in range(self.drag_term_cur.cols) for i in
                                      range(self.drag_term_cur.rows)]
            ...
            self.density_data = preload_density_data(
                os.path.join('utils', 'drag', 'dens_highvar_2000_dens_highvar_2000_lookup.json'))
            self.date_mapping = precompute_date_mapping(pd.to_datetime(self.start_date),
                                                        pd.to_datetime(self.end_date) + pd.DateOffset(
                                                            years=self.simulation_duration))
            ...
            output = solve_ivp(self.population_shell_time_varying_density, [self.scen_times[0], self.scen_times[-1]],
                               x0,
    ...
    def population_shell_time_varying_density(self, t, N, full_lambda, equations, times):
        ...
        if self.time_dep_density:
            ...
            if current_t_step > self.prev_t:
                rho = JB2008_dens_func(t, self.R0_km, self.density_data, self.date_mapping,
                                       self.nearest_altitude_mapping)
                self.prev_rho = rho
                self.prev_t = current_t_step
            else:
                rho = self.prev_rho  # Use cached rho
            # rho_reshape = rho.reshape(-1, 1)  # Convert to column vector
            rho_mat = np.tile(rho, (1, self.species_length))
            rho_mat = sp.Matrix(rho_mat)

            # Second to last row
            upper_rho = rho[1:]
            upper_rho_full = np.tile(upper_rho, self.species_length)

            # First to penultimate row (mimics rho_mat(1:end-1, :))
            current_rho = rho[:-1]
            current_rho_full = np.tile(current_rho, self.species_length)
            ...
            for i in range(len(N)):
                ...
                if i < len(N) - 1:
                    current_drag = self.drag_cur_lamd[i](*N) * current_rho_full[i]
                    upper_drag = self.drag_upper_lamd[i](*N) * upper_rho_full[i]
    ...
    def population_shell(self, t, N, full_lambda, equations, times):
        ...
        for i in range(len(N)):
            ...
            if full_lambda[i] is not None:
                increase = [lambda_shell[0] for lambda_shell in full_lambda[
                                                                i // self.n_shells * self.n_shells:i // self.n_shells * self.n_shells + self.n_shells]]

            #     increase = []
            #     for lam_shell in full_lambda[i]:
            #         print("lam_shell", lam_shell)
            #         increase.append(lam_shell[0])
            #         #increase.append(np.interp(t, times, lam_shell))
            # # Compute the intrinsic rate of change from the differential equation
            # #dN_dt[i] += equations[i](*N) #increase due to launches(launch calculated as interp) + populations

            dN_dt.append(equations[i](*np.concatenate([N, increase])))

        return dN_dt

    def build_numeric_functions(self):
        shells = self.n_shells
        species_list = [x for xs in self.species.values() for x in xs]

        numeric_funcs = {'drag_upper': [], 'drag_current': [], 'collision': [], 'launch': [], 'pmd': []}

        # lambdify drags
        for shell in range(shells):
            curr_funcs_shell = []
            for idx, species in enumerate(species_list):
                curr_sym = species.sym[shell]
    
                eq_cur = self.drag_cur_with_density[shell, idx]
                
                numeric_func_cur = sp.lambdify((curr_sym,), eq_cur, 'numpy')
                curr_funcs_shell.append(numeric_func_cur)

                # print(f"\n[DEBUG] Shell {shell}, Species {species.sym_name}:")
                # print(f"Selected curr eq (species index {idx}): {eq_cur}")
                # print(f"Symbols expected: {curr_sym}")
    
            numeric_funcs['drag_current'].append(curr_funcs_shell)
    
        # Drag upper 
        for shell in range(shells - 1):
            uppr_funcs_shell = []
            for idx, species in enumerate(species_list):
                uppr_sym = species.sym[shell + 1]
    
                eq_uppr = self.drag_upper_with_density[shell, idx]
    
                numeric_func_uppr = sp.lambdify((uppr_sym, ), eq_uppr, 'numpy')
                uppr_funcs_shell.append(numeric_func_uppr)
                
                # print(f"\n[DEBUG] Shell {shell}, Species {species.sym_name}:")
                # print(f"Selected uppeer eq (species index {idx}): {eq_uppr}")
                # print(f"Symbols expected: {uppr_sym}")
    
            numeric_funcs['drag_upper'].append(uppr_funcs_shell)

        # lambdify collision terms
        for shell in range(shells):
            collision_funcs_shell = []
            for idx, pair in enumerate(self.collision_pairs):
                s1_sym, s2_sym = pair.species1.sym[shell], pair.species2.sym[shell]

                # find indices for two species
                species1_idx = species_list.index(pair.species1)
                species2_idx = species_list.index(pair.species2)

                if species1_idx != species2_idx:
                    # lambdify by speecies
                    collision_func_s0 = sp.lambdify((s1_sym, s2_sym), pair.eqs[shell, 0], 'numpy')
                    collision_func_s1 = sp.lambdify((s1_sym, s2_sym), pair.eqs[shell, 1], 'numpy')
                    collision_func_s2 = sp.lambdify((s1_sym, s2_sym), pair.eqs[shell, 2], 'numpy')
                    collision_funcs_shell.append((collision_func_s0, collision_func_s1, collision_func_s2))

                else:  # self-collision
                    collision_func_s0 = sp.lambdify(s1_sym, pair.eqs[shell, 0], 'numpy')
                    collision_func_s1 = sp.lambdify(s1_sym, pair.eqs[shell, 1], 'numpy')
                    collision_func_s2 = sp.lambdify(s1_sym, pair.eqs[shell, 2], 'numpy')
                    collision_funcs_shell.append((collision_func_s0, collision_func_s1, collision_func_s2))
                    
                # print(f"\n[DEBUG] Shell {shell}, Pair {idx} ({pair.species1.sym_name}, {pair.species2.sym_name}):")
                # print(f"All symbolic eq : {pair.eqs[shell, :]}")
                # print(f"Selected symbolic eq (S): {pair.eqs[shell, 0]}")
                # print(f"Selected symbolic eq (N): {pair.eqs[shell, 1]}")
                # print(f"Selected symbolic eq (D): {pair.eqs[shell, 2]}")
                # print(f"Symbols expected: {s1_sym}, {s2_sym}")

            numeric_funcs['collision'].append(collision_funcs_shell)

        # PMD functions
        for shell in range(shells):
            pmd_funcs_shell = []
            for idx, species in enumerate(species_list):
                pmd_sym = species_list[0].sym[shell]
    
                eq_pmd = self.full_Cdot_PMD[shell, idx]
    
                numeric_func_pmd = sp.lambdify((pmd_sym, ), eq_pmd, 'numpy')
                pmd_funcs_shell.append(numeric_func_pmd)
            
            numeric_funcs['pmd'].append(pmd_funcs_shell)
        
        return numeric_funcs
    
    def run_euler_maruyama(self, diffusion_factor=1.0):
        ...
        shells = self.n_shells
        species_list = [x for xs in self.species.values() for x in xs]
        species_len = len(species_list)
    
        populations = np.zeros((shells, species_len, TS))
        # initial population
        for shell in range(shells):
            for i, species in enumerate(species_list):
                populations[shell, i, 0] = self.x0.iloc[shell,i]
    
        numeric_funcs = self.build_numeric_functions()
    
        for j in range(TS - 1):
            rho = self.density_model(j, self.R0_km, self.species, self).reshape(-1)

            for shell in range(shells):
                curr_pop = populations[shell, :, j]
    
                drag_drift_curr = np.array([
                    numeric_funcs['drag_current'][shell][idx](curr_pop[idx]) #* rho[shell]
                    for idx in range(species_len)
                ])
    
                collision_total = np.zeros(species_len)
                collision_diffusion = np.zeros(species_len)
                for idx, pair in enumerate(self.collision_pairs):
                    species1_idx = species_list.index(pair.species1)
                    species2_idx = species_list.index(pair.species2)
                
                    if species1_idx != species2_idx:
                        collision_func_s0, collision_func_s1, collision_func_s2 = numeric_funcs['collision'][shell][idx]
                        collision_val_s0 = collision_func_s0(curr_pop[species1_idx], curr_pop[species2_idx])
                        collision_val_s1 = collision_func_s1(curr_pop[species1_idx], curr_pop[species2_idx])
                        collision_val_s2 = collision_func_s2(curr_pop[species1_idx], curr_pop[species2_idx])

                        collision_total[0] += collision_val_s0
                        collision_total[1] += collision_val_s1
                        collision_total[2] += collision_val_s2
                        
                    else:
                        collision_func_s0, collision_func_s1, collision_func_s2 = numeric_funcs['collision'][shell][idx]
                        collision_val_s0 = collision_func_s0(curr_pop[species1_idx])
                        collision_val_s1 = collision_func_s1(curr_pop[species1_idx])
                        collision_val_s2 = collision_func_s2(curr_pop[species1_idx])
                
                        collision_total[0] += collision_val_s0
                        collision_total[1] += collision_val_s1
                        collision_total[2] += collision_val_s2
                
                    # pair-specific Brownian motion realization
                    dW_coll_raw = np.random.normal(0, np.sqrt(dt))
                
                    # satellite diffusion truncation for each pair (if satellite involved)
                    if collision_val_s0 != 0:
                        satellite_drift = collision_val_s0 * dt
                        satellite_diffusion_coeff = np.sqrt(abs(collision_val_s0))
                        
                        if satellite_diffusion_coeff > 0:
                            dW_satellite_upper_limit = -(satellite_drift) / satellite_diffusion_coeff
                            if dW_coll_raw > dW_satellite_upper_limit:
                                dW_coll = dW_coll_raw#dW_satellite_upper_limit
                            else:
                                dW_coll = dW_coll_raw
                        else:
                            dW_coll = dW_coll_raw
                    ...
                    #print(shell, (collision_val_s0 * dt), satellite_diffusion_coeff, [-(satellite_drift) / satellite_diffusion_coeff], dW_coll_raw, dW_coll)
                
                    # update collision diffusion using this pair-specific Brownian motion
                    drift_sqrt_s0 = np.sqrt(np.abs(collision_val_s0))
                    collision_diffusion[0] += drift_sqrt_s0 * dW_coll

                    drift_sqrt_s1 = np.sqrt(np.abs(collision_val_s1))
                    collision_diffusion[1] += drift_sqrt_s1 * dW_coll
    
                    drift_sqrt_s2 = np.sqrt(np.abs(collision_val_s2))
                    collision_diffusion[2] += drift_sqrt_s2 * dW_coll
                    

                        
                launch_term = np.array([
                    np.interp((j + 1)/(TS/T), np.arange(len(self.full_lambda[species_idx][shell,:])), self.full_lambda[species_idx][shell,:])
                    for species_idx in range(species_len)
                ])
                
                dW_launch = np.random.normal(0, np.sqrt(dt))
                launch_diffusion = np.sqrt(np.abs(launch_term)) * dW_launch

                pmd_term = np.array([
                    numeric_funcs['pmd'][shell][idx](curr_pop[0])
                    for idx in range(species_len)
                ])

                #dW_pmd = np.random.normal(0, np.sqrt(dt), species_len)
                dW_pmd = 0
                dW_pmd_raw = np.random.normal(0, np.sqrt(dt))
                curr_satellite_pop = curr_pop[0]
                satellite_drift = pmd_term[0] * dt
                satellite_diffusion_coeff = np.sqrt(np.abs(pmd_term[0]))
                if satellite_diffusion_coeff > 0:
                    dW_satellite_upper_limit = -(satellite_drift) / satellite_diffusion_coeff
                    #print(shell, (satellite_drift), satellite_diffusion_coeff, [dW_satellite_upper_limit], dW_coll_raw)
                    if dW_pmd_raw > dW_satellite_upper_limit:  # dW_coll이 너무 커서 개체수가 증가하면
                        dW_pmd = dW_pmd_raw #dW_satellite_upper_limit  # 상한으로 제한
                    else:
                        dW_pmd = dW_pmd_raw  # 제한하지 않아도 됨
                else:
                    dW_pmd = dW_pmd_raw
                pmd_diffusion = np.sqrt(np.abs(pmd_term)) * dW_pmd
    
                if shell < shells - 1:
                    uppr_pop = populations[shell + 1, :, j]
                    drag_drift_uppr = np.array([
                        numeric_funcs['drag_upper'][shell][idx](uppr_pop[idx]) #* rho[shell+1]
                        for idx in range(species_len)
                    ])

                    dW_drag_uppr = np.random.normal(0, np.sqrt(dt), species_len)
                    drag_diffusion_uppr = np.sqrt(np.abs(drag_drift_uppr)) * dW_drag_uppr

                    dW_drag_curr = np.random.normal(0, np.sqrt(dt), species_len)
                    drag_diffusion_curr = np.sqrt(np.abs(drag_drift_curr)) * dW_drag_curr

                    # drift_diffusion_total = drag_drift_curr*dt + drag_diffusion_curr + drag_drift_uppr*dt + drag_diffusion_uppr + (collision_total*dt + collision_diffusion) + (launch_term*dt + launch_diffusion) + pmd_term*dt + pmd_diffusion

                    drift_diffusion_total = (
                        drag_drift_curr + drag_drift_uppr + collision_total + launch_term + pmd_term
                    ) * dt + diffusion_factor * (drag_diffusion_curr + drag_diffusion_uppr + collision_diffusion + launch_diffusion + pmd_diffusion)
                    
                else:
                    dW_drag_curr = np.random.normal(0, np.sqrt(dt), species_len)
                    drag_diffusion_curr = np.sqrt(np.abs(drag_drift_curr)) * dW_drag_curr

                    # drift_diffusion_total = drag_drift_curr*dt + drag_diffusion_curr + (collision_total*dt + collision_diffusion) + (launch_term*dt + launch_diffusion) + pmd_term*dt + pmd_diffusion

                    
                    drift_diffusion_total = (
                        drag_drift_curr + collision_total + launch_term + pmd_term
                    ) * dt + diffusion_factor * (drag_diffusion_curr + collision_diffusion + launch_diffusion + pmd_diffusion)
    
                populations[shell, :, j + 1] = np.maximum(0, populations[shell, :, j] + drift_diffusion_total)
                
            res_list = populations.transpose(1, 0, 2).reshape(len(species_list) * shells, TS)
